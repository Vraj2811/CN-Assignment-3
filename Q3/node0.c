...
void calc_send_pkt0()
{
    ...
    // New inline min cost calculation replacing calc_min_cost0
    for (int i = 0; i < 4; i++)
    {
        int min_val = INFINITY;
        for (int j = 0; j < 4; j++)
        {
            if (dt0.costs[i][j] < min_val)
                min_val = dt0.costs[i][j];
        }
        min_cost_0[i] = min_val;
    }
    ...
    { // min cost changed, so send new packets
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            { // not sending self
                tolayer2(pkt0[i]);
                printf("At time t=%.3f, node %d sends packet to node %d with: (%d  %d  "
                       "%d  %d)\n",
                       clocktime, pkt0[i].sourceid, pkt0[i].destid, pkt0[i].mincost[0],
                       pkt0[i].mincost[1], pkt0[i].mincost[2], pkt0[i].mincost[3]);
            }
        }
    }
    else
...
void rtinit0()
{
    ...
    // Replacing calc_min_cost0 with inline min cost calculation
    for (int i = 0; i < 4; i++)
    {
        int min_val = INFINITY;
        for (int j = 0; j < 4; j++)
...
void rtupdate0(rcvdpkt) struct rtpkt *rcvdpkt;
{
    ...
    printf("rtupdate0() is called at time t=: %0.3f as node %d sent a pkt with "
           "(%d  %d  %d  %d)\n",
...
printmincost0()
{
    printf("Minimum cost from %d to other nodes are: %d %d %d %d\n", 0,
           min_cost_0[0], min_cost_0[1], min_cost_0[2], min_cost_0[3]);
}
...

{
    ...
    {
        dt0.costs[i][linkid] =
            new_dist_0_to_linkid +
            old_dist_linkid_to_others[i]; // = new_dist_0_to_i (0 to i via linkid)
    }
